...
2. For the case where we want to evaluate single responses in the webapp, as it was discussed previously, there is a possibility of coordinated responses appearing in a sentence. To remedy that, I am using the get_np function from the dev which returns a tuple with the actual noun_phrase as tuple[0], and whether coordination was found or not as a boolean in tuple[1]. Passing a list to the evaluation_transformer function would fail so the program checks whether tuple[1] is true (there is coordination) or not (there is no coordination) and either runs the evaluation function for the single response (case 2) or does a for loop and evaluates each response (1).
