...
# Needed for the NLP models loaded in the rorschach app.py
...
def y_boolean(probabilities, classification):
    ...
    if 1 not in y_pred:
        ...
        y_pred[idx] = 1
        st.write("Low confidence prediction for {}".format(name_class))
...
def get_responses(raw_text):
  ...
  report = st.write("Report parsed successfully!")
  for i in range(len(list_number_figure)):
    ...
    st.write("Figure", dict_responses["figure_number"])
    for dictionary in dict_responses["sentences"]:
      ...
      for key in list_keys:
        if "response" == key:
          ...
          st.write("Canonical Determinants:", dictionary["individual_determinants"], "\nMacro Determinants:", dictionary["macro_determinants"], "\n")            
